...
export type VariantEnrichmentBarPlotProps = {
...
const VariantEnrichmentBarPlot = ({ data, selected, sortedFilteredData, ...rest }: VariantEnrichmentBarPlotProps) => {
          ...
          return (
            {
              ...
              color: (anySelected && isSelected || !anySelected) ? tissueColors[x.ontology] ?? tissueColors.missing : '#CCCCCC',
              id: i.toString(),
              metadata: x,
              lollipopValue: x.fdr
            }
  ...
  return (
    ...
    >
      <BarPlot barSize={3} barSpacing={20} legendTitle={"FDR"} {...rest} data={plotData} topAxisLabel={`Log2(Fold Enrichment)`} TooltipContents={(bar)=>{
        return <><div style={{ padding: 2 }}>
        <strong>{bar.label}</strong>
        <br/>
        {bar.metadata && (
            <>
                <div>FDR: {bar.metadata.fdr.toFixed(3)}</div>
...
export default VariantEnrichmentBarPlot;
