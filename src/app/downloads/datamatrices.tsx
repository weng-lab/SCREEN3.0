...
import { BiosampleTable, DataTable, DataTableColumn, EncodeBiosample, isAutogeneratedRow } from "@weng-lab/ui-components";
...
export function DataMatrices() {
  ...
  const [searchedBiosample, setSearchedBiosample] = useState<string>(null);
  ...
  const handleSetSelectedAssay = (newSelected: Selected) => {
    if (JSON.stringify(newSelected.assay) !== JSON.stringify(selectedAssay.assay)) {
      setSelectedBiosamples([]); // clear umap selection when assay changes
      setSearchedBiosample(null); // clear table selection when assay changes
  ...
  const handleSetSearchedSample = (biosample: [EncodeBiosample]) => {
    if (biosample[0]?.displayname) {
      setSearchedBiosample(biosample[0].displayname);
    } else setSearchedBiosample(null);
  };
  ...
  const scatterData: Point<PointMetaData>[] = useMemo(() => {
    if (!fData) return [];
    const biosampleIds = selectedBiosamples.map((sample) => sample.umap_coordinates);
    ...
    return fData.map((x) => {
      ...
      return {
        ...
        r: searchedBiosample && x.displayname === searchedBiosample ? 5 : 2,
        color:
          searchedBiosample === null || x.displayname === searchedBiosample
  ...
  }, [fData, searchedBiosample, colorBy, sampleTypeColors, ontologyColors, selectedBiosamples]);
  ...
  return (
    <Grid container mt={1} direction="column" sx={{ paddingX: 5, height: "100%", minHeight: 0 }}>
      <Stack direction="row" spacing={10} sx={{ minHeight: 0 }}>
        <Stack direction="column" spacing={2} flex={1}>
          ...
          >
            ...
            >
              <Button endIcon={selectedBiosamples.length !== 0 && <Visibility />} onClick={handleOpenModal}>
                {`${selectedBiosamples.length} Experiments Selected`}
              </Button>
              <Button onClick={() => setSelectedBiosamples([])}>Clear Selection</Button>
            </Stack>
            <ScatterPlot
        ...
        <Stack flex={1} minHeight={0}>
          {searchedBiosample && (
            <Paper sx={{ mb: 1 }}>
              ...
              >
                ...
                <IconButton onClick={() => setSearchedBiosample(null)} sx={{ m: "auto", flexGrow: 0 }}>
                  <CancelRounded />
                </IconButton>
              </Stack>
            </Paper>
          ...
          <BiosampleTable
            ...
            onSelectionChange={handleSetSearchedSample}
