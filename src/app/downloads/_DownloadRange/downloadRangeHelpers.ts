...
export type GenomicRegion = { chromosome: string; start: number; end: number };

/**
 *
 * @param input A user-entered string representing a genomic region; chr_:start-end or tab separated format supported
 * @returns the parsed region, or null if the input isn't a complete, well formed region
 */
export function parseGenomicRegion(input: string): GenomicRegion | null {
  const trimmed = input.trim();
  //If input contains a tab character split on it, else assume it's the chr:start-end format
  const [chromosome, ...rest] = trimmed.includes("\t") ? trimmed.split("\t") : trimmed.split(/[:-]/);
  const [start, end] = rest.map((coordinate) => coordinate.replace(/,/g, "").trim());

  //Coordinates are only valid if they're present and contain nothing but digits
  if (!chromosome || !/^\d+$/.test(start ?? "") || !/^\d+$/.test(end ?? "")) {
    return null;
  }

  return { chromosome, start: +start, end: +end };
}
