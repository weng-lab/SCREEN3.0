...
export const DownloadButton = <T extends boolean>({ row, downloadType }: DownloadButtonProps<T>) => {
  ...
  switch (downloadType) {
    ...
    case "atac":
      ...
      break;
    case "celltypeccres": {
      const signalIDs = [row.dnase_signal, row.h3k4me3_signal, row.h3k27ac_signal, row.ctcf_signal].filter(
  ...
  if (
    row[downloadType] ||
    (downloadType === "celltypeccres" && (row.dnase || row.ctcf || row.h3k27ac || row.h3k4me3))
  ) {
    return (
