...
export const BiosampleTables = <HasRNASeq extends boolean = false, AllowMultiSelect extends boolean = false>({
  ...
  const handleSetMustHaveRnaSeq = (newState: boolean) => {
    setMustHaveRnaSeq(newState);
  };
  ...
  const {
    data: biosampleData,
    loading: loadingBiosamples,
    error: errorBiosamples,
  ...
  const {
    data: data_rnaseq,
    loading: loading_rnaseq,
    error: error_rnaseq,
  } = useQuery(RNA_SEQ_QUERY, {
    variables: {
      assembly: assembly.toLowerCase() as "grch38" | "mm10",
    },
    skip: !showRNAseq,
      ...
      if (onChange && typeof onChange === "function") {
        if (allowMultiSelect) {
          ...
          if (selectedSamples.some((sample) => sample.name === clicked.name)) {
            ...
            (onChange as (selected: BiosampleData<HasRNASeq>[]) => void)(
              selectedSamples.filter((x) => x.name !== clicked.name)
            );
          } else {
    ...
    const handleModifyAll = (samples: BiosampleData<HasRNASeq>[], action: "select" | "deselect") => {
      if (onChange && typeof onChange === "function") {
        if (action === "select") {
          ...
          (onChange as (selected: BiosampleData<HasRNASeq>[]) => void)(
            selectedSamples.filter((x) => !toRemove.includes(x))
          );
        }
      }
    };

    return Object.entries(filteredBiosamples)
      ...
      .map(([ontology, biosamples]) => {
        ...
        const selectAllIndeterminate =
          biosamples.some((sample) => selectedSamples.some((x) => x.name === sample.name)) && !selectAllChecked;
        ...
        return (
          <Accordion key={ontology} {...accordionProps}>
            <AccordionSummary
              ...
              sx={{
                ...
                color: highlighted || selected ? theme.palette.primary.main : "text.primary",
            ...
            >
              <Stack width={"100%"} direction={"row"} alignItems={"center"} justifyContent={"space-between"}>
                {biosamples.length !== unfilteredBiosamples[ontology].length ? (
                  <Typography>
                    ...
                    <span style={{ opacity: 0.5 }}>
                      <s>{unfilteredBiosamples[ontology].length}</s>
                    </span>
                    )
                  </Typography>
                ) : (
                ...
                {showDownloads && assembly !== "mm10" && <AggregateDownloadButton ontology={ontology} />}
              </Stack>
            </AccordionSummary>
            <AccordionDetails>
              <DataTable
  ...
  const handleResetFilters = useCallback(() => {
    ...
    handleSetLifeStageFilter(
      Object.fromEntries(Object.entries(lifeStageFilter).map(([key]) => [key, true])) as LifeStageCheckboxes
    );
    handleSetSampleTypeFilter(
      Object.fromEntries(Object.entries(sampleTypeFilter).map(([key]) => [key, true])) as SampleTypeCheckboxes
    );
    handleSetCollectionFilter(
      Object.fromEntries(Object.entries(collectionFilter).map(([key]) => [key, true])) as CollectionCheckboxes
    );
    ...
    <Stack component={Paper} height={500} {...slotProps?.paperStack}>
      ...
      >
        <Stack px={2} pt={1} gap={1} {...slotProps?.menuStack}>
            ...
            <FormControl component="fieldset" variant="standard">
              ...
              <FormGroup>
                <FormControlLabel
                  ...
                  control={
                    <Checkbox checked={mustHaveRnaSeq} onChange={(e) => handleSetMustHaveRnaSeq(e.target.checked)} />
                  }
                />
              </FormGroup>
            </FormControl>
          ...
          <FilterCheckboxGroup
            groupLabel="Biosample Types"
            controlsState={sampleTypeFilter}
            setState={handleSetSampleTypeFilter}
          />
          <FilterCheckboxGroup
            groupLabel="Collection"
            controlsState={collectionFilter}
            setState={handleSetCollectionFilter}
          />
          <FilterCheckboxGroup
            groupLabel="Life Stage"
            controlsState={lifeStageFilter}
            setState={handleSetLifeStageFilter}
          />
