...
export const DownloadButtonGroup = ({ ontology }: DownloadButtonGroupProps) => {
  ...
  const handleClose = (event: Event) => {
    if (buttonGroupAnchor.current && buttonGroupAnchor.current.contains(event.target as HTMLElement)) {
      return;
    }
  ...
  return (
    <>
      ...
      >
        ...
        <Tooltip
          title={availibleDownloads.length === 0 ? "No Download Options" : `Download Options`}
          placement="top"
          arrow
        >
          <span>
              ...
              <ClickAwayListener onClickAway={handleClose}>
                <MenuList id="split-button-menu" autoFocusItem>
                  {downloadOptions.map((type) => {
                    const filename =
                      type === "Non-cancerous Lines" ? ncFile : type === "All Cell Lines" ? allFile : undefined;
