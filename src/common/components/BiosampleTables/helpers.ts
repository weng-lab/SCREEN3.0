...
function hasRNASeq<HasRNASeq extends boolean>(biosample: BiosampleData<HasRNASeq>, mustHaveRnaSeq: boolean): boolean {
  if (mustHaveRnaSeq) {
  ...
  for (const ontology in biosamples) {
    filteredBiosamples[ontology] = biosamples[ontology].filter((biosample) => {
      ...
      if (sampleTypeFilter["Tissue"] && biosample.sampleType === "tissue") {
        ...
        sampleTypeFilter["In Vitro Differentiated Cells"] &&
        biosample.sampleType === "in vitro differentiated cells"
      ) {
        passesType = true;
      ...
      const passesGlobalSearch = globalSearch
        ? Object.values(biosample).some(
            (val) => typeof val === "string" && val?.toLowerCase().includes(globalSearch.toLowerCase())
          )
        : true;
 ...
 */
export function assayHoverInfo(assays: {
  dnase: boolean;
  h3k27ac: boolean;
  h3k4me3: boolean;
  ctcf: boolean;
