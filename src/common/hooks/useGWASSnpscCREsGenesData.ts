...
export const useGWASSnpscCREsGenesData = ({
  study,
  method,
}: useGWASSnpscCREsGenesParams): useGWASSnpscCREsGenesReturn => {
  const {
    data: gwasstudySNPs,
  ...
  const uniqueAccessions =
    cCREIntersections && cCREIntersections.intersection.length > 0 ? [...new Set(studycCREs)] : [];

  const {
    data: studyGenes,
    loading: studyGenesLoading,
    error: studyGenesError,
  } = useQuery(LINKED_GENES, {
    variables: {
      assembly: "grch38",
